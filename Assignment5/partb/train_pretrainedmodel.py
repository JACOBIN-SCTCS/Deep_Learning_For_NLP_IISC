...
def collate_function(batch_data):
    ...
    input_text = [ b['input_text'] for b in batch_data]
    output_text =  [ b['output_text'] for b in batch_data]

    input_text_ids = torch.stack([ b['input_text_ids'] for b in batch_data])
    input_attention_mask = torch.stack([b['input_attention_mask'] for b in batch_data])
    output_text_ids = torch.stack([b['output_text_ids'] for b in batch_data])
    
    
    output_attention_mask = torch.stack([b['output_attention_mask'] for b in batch_data])
...
valid_dataloader = DataLoader(valid_dataset,32,shuffle=True,collate_fn=collate_function)
...
class TransformerTranslator(pl.LightningModule):
    ...
    def training_step(self, batch_data,batch_idx):
        ...
        final_answers = postfix_evaluation(answers,batch_data['current_input_numbers']).to(device_fast)


        regression_loss = torch.min(F.mse_loss(final_answers,batch_data['current_outputs']),torch.tensor(1.0))
        #train_loss = self.loss_fn(predictions,output_expected)
        train_loss = train_loss + 0.01*regression_loss
    ...
    def validation_step(self, batch_data,batch_idx):
        ...
        regression_loss = torch.min(F.mse_loss(final_answers,batch_data['current_outputs']),torch.tensor(1.0))
        #train_loss = self.loss_fn(predictions,output_expected)
        valid_loss = valid_loss + 0.01*regression_loss
...
checkpoint_callback = ModelCheckpoint(
    dirpath = "checkpoints",
    filename="transformer-scratch-output-agonostic-best-checkpoint",
    save_top_k = 1,
    verbose = True,
    monitor="valid_loss",
    mode = "min"
...
logger = TensorBoardLogger("transformer_scratch_output_agonostic_logs",name="transformertranslator")
